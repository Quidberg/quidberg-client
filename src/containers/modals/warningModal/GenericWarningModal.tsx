...
const GenericWarningModal = ({isOpen, close, proceed, proceedButtonText, warningMsg, headerText}:WarningProps) => {
  return (
    <Modal isOpen={isOpen} close={close}>
        <div className="flex flex-col items-center justify-center p-3 py-4 md:p-8 gap-7">
            ...
            <section className="mt-8 text-center flex flex-col gap-4 ">
                {headerText && <h1 className="font-semibold text-[18px] md:text-lg text-main_bg hover:text-main_bg/70" >{headerText}</h1>}
            ...
            <section className="w-full flex gap-4 justify-center">
                ...
                <RoundedButton onClick= {proceed}>{proceedButtonText || 'Proceed'}</RoundedButton>
            </section>
        </div>
    </Modal>
  )
