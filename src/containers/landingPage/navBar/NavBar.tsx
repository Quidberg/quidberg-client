...
const NavBar = ({ handleAuth, goHome, ref, isScrolledDown}: Props) => {
  return (
    <nav className={`z-nav_bar ${isScrolledDown && 'backdrop-blur-md shadow-light_gray_bg/10 bg-light_pry_bg/3'} flex items-center px-5 py-2 `} ref={ref}>
      <button
      ...
      <div className="flex flex-1 justify-end sm:justify-between">
        ...
        <div className="flex flex-row items-center pl-4 text-sm md:text-base gap-3  ">
          ...
          {/* sign in */}
          <button onClick={() => handleAuth(AuthType.signin)} className="text-white/80">Sign In</button>
